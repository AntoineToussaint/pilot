...
impl Session {
    ...
    /// Mark all current activity as read.
    ...
    /// Whether this session is currently snoozed.
    ...
    pub fn action_priority(&self, my_username: &str) -> ActionPriority {
        ...
        if let time::Staleness::Stale { .. } | time::Staleness::Abandoned { .. } =
            time::staleness(&task.updated_at, &task.updated_at)
        {
...
mod tests {
    ...
    fn test_session_defaults() {
        let s = Session::new(make_task("Fix bug"));
    ...
    fn test_activity_and_unread() {
        let mut s = Session::new(make_task("PR"));
    ...
    fn test_priority_ci_failed() {
        ...
        let s = Session::new(t);
        assert_eq!(s.action_priority("me"), ActionPriority::CiFailed);
    }
    ...
    fn test_priority_approved() {
        ...
        let s = Session::new(t);
        assert_eq!(s.action_priority("me"), ActionPriority::ApprovedReadyToMerge);
    }
    ...
    fn test_priority_needs_review() {
        ...
        let s = Session::new(t);
        assert_eq!(s.action_priority("me"), ActionPriority::NeedsYourReview);
    }
