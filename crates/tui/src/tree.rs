...
/// Owner of all mounted components and the single keyboard-focus
/// cursor.
///
/// ## Focus invariant
///
/// At most ONE component is focused at any time, identified by a
/// single `Option<ComponentId>`. There is no "the sidebar pane is
/// focused" abstraction — focus targets a leaf component. Two calls
/// to `set_focus` are mutually exclusive: the second wins.
///
/// `focused()` returns `None` when nothing is focused (e.g. during
/// a mouse drag the app temporarily owns input). The default
/// post-construction state is `Some(root)` so the bubble-up dispatch
/// has a well-defined target. Use `unfocus()` to opt out.
...
impl ComponentTree {
    ...
    pub fn unmount(&mut self, id: ComponentId) {
        ...
        let focus_was_in_subtree = self.focus.is_some_and(|f| subtree.contains(&f));
        ...
        if focus_was_in_subtree {
            self.focus = Some(parent_id.unwrap_or(self.root));
        }
    ...
    /// True when no component currently holds focus.
    pub fn is_unfocused(&self) -> bool {
        self.focus.is_none()
    }

    /// Root → … → leaf. Empty when nothing is focused. Otherwise root
    /// is element 0 and the focus leaf is the last element.
    pub fn focus_path(&self) -> Vec<ComponentId> {
        let Some(leaf) = self.focus else {
            return Vec::new();
        };
    ...
    /// Clear focus. The next keystroke bubbles to nothing — a sane
    /// "the app is doing something modal that isn't a component" idle
    /// state. Used by mouse drag operations and overlays that own
    /// input directly.
    ...
    fn cycle_sibling(&mut self, dir: isize) {
        ...
        let Some(parent) = self.parent.get(&focus).copied() else {
            return;
        };
        ...
        let Some(pos) = siblings.iter().position(|c| *c == focus) else {
            return;
        };
