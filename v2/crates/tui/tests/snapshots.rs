...
use pilot_core::{CiStatus, ReviewStatus, Session, Task, TaskId, TaskRole, TaskState};
...
fn sidebar_golden_render_focused() {
    ...
    // Build three sessions with known ages so sort order is
    // deterministic in the snapshot.
    s.on_event(&Event::Snapshot {
        sessions: vec![
            Session::new_at(make_task("o/r#1", 10), fixed_time()),
            Session::new_at(make_task("o/r#2", 60), fixed_time()),
            Session::new_at(make_task("o/r#3", 120), fixed_time()),
        ],
        terminals: vec![],
    });
...
fn sidebar_golden_render_unfocused() {
    let mut s = Sidebar::new(ComponentId::new(1));
    s.on_event(&Event::Snapshot {
        sessions: vec![Session::new_at(make_task("o/r#1", 10), fixed_time())],
        terminals: vec![],
    });
