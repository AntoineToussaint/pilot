...
//! ## Data flow
//!
//! AppRoot reads `sidebar.selected_session()` after every key event
//! and calls `right_pane.set_session(...)`. The RightPane doesn't
//! track every session the daemon knows about — only the currently
//! selected one. This keeps the component simple and its event
//! handler a no-op.

use crate::{Component, ComponentId, Outcome};
use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};
use pilot_core::{ActivityKind, CiStatus, ReviewStatus, Session, TaskRole};
...
impl RightPane {
    ...
    fn render_header(&self, area: Rect, frame: &mut Frame) {
        ...
        // Line 2: role + branch
        let role_label = match task.role {
            TaskRole::Author => ("Author", Color::Cyan),
            TaskRole::Reviewer => ("Reviewer", Color::Magenta),
            TaskRole::Assignee => ("Assignee", Color::Green),
            TaskRole::Mentioned => ("Mentioned", Color::DarkGray),
        };
        let branch = task.branch.as_deref().unwrap_or("-");
        lines.push(Line::from(vec![
            Span::raw("Role: "),
            Span::styled(role_label.0, Style::default().fg(role_label.1)),
            Span::raw("   Branch: "),
            Span::styled(branch, Style::default().fg(Color::Cyan)),
        ]));

        // Line 3: CI + review
        let ci_str = match task.ci {
            CiStatus::Success => ("PASS", Color::Green),
            CiStatus::Failure => ("FAIL", Color::Red),
            CiStatus::Running | CiStatus::Pending => ("RUN", Color::Yellow),
            CiStatus::Mixed => ("MIX", Color::Yellow),
            CiStatus::None => ("--", Color::DarkGray),
        };
        let review_str = match task.review {
            ReviewStatus::Approved => ("Approved", Color::Green),
            ReviewStatus::ChangesRequested => ("Changes", Color::Red),
            ReviewStatus::Pending => ("Pending", Color::Yellow),
            ReviewStatus::None => ("--", Color::DarkGray),
        };
        lines.push(Line::from(vec![
            Span::raw("CI: "),
            Span::styled(ci_str.0, Style::default().fg(ci_str.1)),
            Span::raw("   Review: "),
            Span::styled(review_str.0, Style::default().fg(review_str.1)),
        ]));

        // Line 4: reviewers
        let reviewers_text = if task.reviewers.is_empty() {
            "none".to_string()
        } else {
            task.reviewers.join(", ")
        };
        lines.push(Line::from(vec![
            Span::raw("Reviewers: "),
            Span::styled(reviewers_text, Style::default().fg(Color::White)),
        ]));
    ...
    fn render_comments(&self, area: Rect, frame: &mut Frame, focused: bool) {
        ...
        let count = session.activity.len();
...
impl Component for RightPane {
    ...
    fn handle_key(&mut self, key: KeyEvent, _cmds: &mut Vec<Command>) -> Outcome {
        // Comment navigation. Requires a session, otherwise there's
    ...
    fn on_event(&mut self, event: &Event) {
        // When the currently-selected session is upserted, refresh our
        // local copy so comment-cursor offsets stay in range.
        let Event::SessionUpserted(session) = event else {
            return;
        };
